...
async def send_message_email(
    ...
    The new message is surfaced at the top so the submitter sees the question
    without scrolling; the prior conversation (older messages newest-first,
    original submission at the bottom) follows for context. Replies happen
    in CYM via the lookup-token link, not by email reply.

    `prior_thread` is a list of `{"direction": "admin"|"submitter", "body": str,
    "created_at": datetime}` dicts containing every message EXCEPT the new one
    being sent — the new message is passed separately as `message_body` and
    rendered at the top.
    """
    if not settings.RESEND_API_KEY or not settings.FROM_EMAIL:
    ...
    text_lines = [
        "Hi,",
        "",
        f'We have a message for you about your ticket {display_id} ("{title}"):',
        "",
        message_body.strip(),
        "",
    ...
    for msg in reversed(prior_thread):
        sender = "Erik" if msg["direction"] == "admin" else "You"
        text_lines.extend([
            "",
            f"{sender} · {_fmt_dt(msg['created_at'])}",
...
<p>We have a message for you about your request <strong>"{html_lib.escape(title)}"</strong>:</p>
<blockquote style="margin:16px 0;padding:12px 16px;border-left:4px solid #2563eb;background:#f6f6f3;white-space:pre-wrap;font-size:14px">{html_lib.escape(message_body)}</blockquote>
...
<p style="color:#6b6b65;font-size:13px">Conversation history (most recent first):</p>
{history_html}
</body></html>"""
